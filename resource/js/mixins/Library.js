  ...
  methods: {
    ...
    getForm(refName = "form") {
      ...
      return {
        isValid: () => $form.valid(),
        isValidAsync: () =>
          new Promise(resolve => ($form.valid() ? resolve() : "")),
        getObject: () => $form,
      ...
      return {
        ...
        deleteConfirm() {
          return new Promise(resolve =>
            $confirm("確定要刪除嗎?").then(isOK => (isOK ? resolve() : ""))
          );
        }
      };
    },
...
function imageSize(file, max_width, max_height, min_width, min_height) {
  return new Promise((resolve, reject) => {
    const image = new Image();
    image.src = getURL(file);
    image.onload = () => {
      const messages = [];
      if (max_width && image.width > max_width) {
        messages.push(`圖片寬度不可超過${width}px`);
      }
      if (max_height && image.height > max_height) {
        messages.push(`圖片高度不可超過${height}px`);
      }
      if (min_width && image.width > min_width) {
        messages.push(`圖片寬度不可小於${width}px`);
      }
      if (min_height && image.height > min_height) {
        messages.push(`圖片高度不可小於${height}px`);
      }
      if (messages.length) {
        // const errMessage = messages.join("<br>")
        // Vue.$alert.error(errMessage)
...
function number(value, fixed = 0) {
  ...
  const paddingRight = (str, length) =>
    str.length >= lenght ? str : paddingRight(str + "0", length);
...
function requestSuccess(fn) {
  return new Promise(resolve =>
    fn.then(res => (res.returnCode === 0 ? resolve(res) : ""))
  );
}
